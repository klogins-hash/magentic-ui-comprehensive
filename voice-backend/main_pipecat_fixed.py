...
Updated for Pipecat 0.0.87 API
"""
...
async def websocket_endpoint(websocket: WebSocket):
    ...
    try:
        if voice_pipeline:
            ...
            # Create pipeline
            pipeline = await voice_pipeline.create_pipeline(transport)
...
# Simple text endpoint for testing without voice
@app.post("/api/text")
async def process_text(data: dict):
    """Process text message for testing"""
    message = data.get("content", "")
    
    # Simple delegation logic for testing
    delegation_keywords = ['create', 'generate', 'make', 'build', 'design', 'analyze', 'research', 'write', 'develop', 'automate']
    needs_delegation = any(keyword in message.lower() for keyword in delegation_keywords)
    
    if needs_delegation:
        response = f"I've assigned '{message}' to your AutoGen team."
    else:
